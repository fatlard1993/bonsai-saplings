#!/usr/bin/env python3
"""Generate Bonsai Saplings' mod menu icon: a pot, and a tree that fits in it.

A flower pot in the vanilla pot's own clay colours, a short trunk of oak log
leaning out of it, and a flat pad of oak leaves off to one side, because that
is the shape a bonsai is cut into and a sapling never grows into on its own.
The leaves are tinted the plain oak green the game would give them. Source
pixels are read straight out of the vanilla Minecraft jar and scaled nearest
...
OUT = os.path.join(HERE, "src/main/resources/assets/bonsai-saplings-justfatlard/icon.png")
...
FOLIAGE = (72, 181, 24)   # the oak foliage tint the game applies to grey leaves

# The pad: one row of leaf cells per line, x offsets on a 16 wide sprite.
PAD = [
    (2, range(6, 12)),
    (3, range(3, 14)),
    (4, range(1, 15)),
...
TRUNK = [(6, 9), (7, 9), (8, 8), (9, 8), (10, 7)]   # (y, x), leaning right as it rises
POT_RIM = (11, range(3, 13))
POT_BODY = [(12, range(4, 12)), (13, range(4, 12)), (14, range(5, 11)), (15, range(5, 11))]


def tint(px):
    return tuple(px[i] * FOLIAGE[i] // 255 for i in range(3)) + (px[3],)


def clay_palette():
    """The pot texture's own three tones, darkest first: the rim and shadow
    side, the body, the lit side."""
    counts = Counter(px for row in vanilla("block/flower_pot.png") for px in row if px[3])
    tones = sorted((px for px, _ in counts.most_common(4)),
...
def build_icon():
    sprite = blank()
    dark, clay, lit = clay_palette()
    y, xs = POT_RIM
    for x in xs:
        sprite[y][x] = dark
    for y, xs in POT_BODY:
        for x in xs:
            sprite[y][x] = lit if x == xs.start else dark if x == xs.stop - 1 else clay
    log = vanilla("block/oak_log.png")
    for y, x in TRUNK:
        sprite[y][x] = log[y][x]
        sprite[y][x + 1] = shade(log[y][x + 1], 0.8)
    leaves = [[tint(px) for px in row] for row in vanilla("block/oak_leaves.png")]
    for y, xs in PAD:
        for x in xs:
            px = leaves[y][x]
            sprite[y][x] = px if px[3] else tint(leaves[(y + 3) % 16][(x + 5) % 16])
